...
function RenderStrip({ eleHeight, value }: { eleHeight: string | null, value: string }) {
    ...
    // Track the previous value across renders without reading a ref during render.
...
function AnimatedScore({ value, duration = 0.2, className }: { value: number, duration?: number, className?: string }) {
    ...
    const variants = value >= prevValue ? forwards : backwards
    const direction = value >= prevValue ? "forwards" : "backwards"
...
function ScoreContainer({ number, variants, duration = 0.7 }: {
    number: string,
    variants: Variants,
    duration?: number,
    direction: string
